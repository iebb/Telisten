...
- optional music-search queries and interactive selections sent to Telegram bots configured by the user;
...
## Device and iCloud Keychain data

Raw Telegram authorization keys are stored only in the device-local Keychain and are never synchronized between devices. Audio, artwork, chat music counts, queues, preferences, and lyrics are cached on-device. The account descriptor needed to present the account switcher and user-created search-bot configurations may synchronize through the user's private iCloud Keychain; a new device must still create its own Telegram authorization session. Kitta Ltd cannot access this Keychain data. On-device-only processing and storage are not an App Privacy “collection” unless the data is transmitted off-device.
